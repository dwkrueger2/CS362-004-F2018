...
/*Returns a textual representation of the current hand (the player's hand)*/
string getHandString(int player_i, gameState * state)
{
	stringstream ss;
	ss << "{ ";
	for (int i = 0; i < state->handCount[player_i]; i++)
		ss << getCardName((CARD)state->hand[player_i][i]) << (i < (state->handCount[player_i] - 1) ? ", " : "");
	ss << " }";
	return ss.str();
}
