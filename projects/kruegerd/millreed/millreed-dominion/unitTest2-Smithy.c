// Smithy Card Unit Test - from Assignment 3
...
int main() {
	...
	//		T1  Hand card count should increase by 3
	tResult = (G.handCount[0] == G_original.handCount[0] + 2);
	allPass = allPass && tResult;
	cout << "T1: Smithy Card Test:" << PASS(tResult) << " Returns true when handCount increases by 2\n";
	...
	allPass = allPass && tResult;
	cout << "T2: Smithy Card Test:" << PASS(tResult) << "  Returns true when deckCount decreases by 2 if no shuffle\n";
